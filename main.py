...
dict_vehicles = {"cars": {}, "bikes": {}}

structure = Structure("E_start", "E2", edges, net, dict_vehicles["bikes"], traci, args.config, dict_scenario, simu_length, dict_edges_index=dict_edges_index,\
    ...
    if(True): #new_scenario
        if(step<simu_length):
            for _ in range(bike_poisson_distrib[int(step)]):
                ...
                dict_scenario["bikes"][str(id_cyclist)] = {"start_step": step, "start_edge": e1.getID(), "end_edge": e2.getID(), "max_speed": max_speed,
                "distance_travelled": net.getShortestPath(net.getEdge("E_start"), e2, vClass='bicycle', fromPos=0)[1], "waiting_time": 0}
                spawn_cyclist(id_cyclist, step, path, net, structure, step_length, max_speed, False, args, dict_vehicles["bikes"])
                id_cyclist+=1
        ...
        if(step<simu_length):  
            for _ in range(car_poisson_distrib[int(step)]):
                ...
                dict_scenario["cars"][str(id_car)] = {"start_step": step, "start_edge": e1.getID(), "end_edge": e2.getID(),
        ...
        if(id_cyclist<len(dict_scenario["bikes"]) and step >= dict_scenario["bikes"][id_cyclist]["start_step"]):
            ...
            path = net.getShortestPath(e1, e2, vClass='bicycle')[0]
            spawn_cyclist(id_cyclist, step, path, net, structure, step_length, dict_scenario["bikes"][id_cyclist]["max_speed"], False, args, dict_vehicles["bikes"])
            id_cyclist+=1
    ...
    for vehicle_type in dict_vehicles:
        for i in copy.deepcopy(list(dict_vehicles[vehicle_type].keys())):
            if(vehicle_type == "bikes"):
                sumo_id = str(i)
                dict_vehicles[vehicle_type][i].step(step)
            elif(vehicle_type == "cars"):
                sumo_id = str(i)+"_c"

            vehicles_in_simu  = traci.vehicle.getIDList()
            if(dict_scenario[vehicle_type][i]["start_step"]<0 and i in vehicles_in_simu and traci.vehicle.getRoadID(sumo_id) == "E_start"):
                dict_scenario[vehicle_type][i]["start_step"] = step

            if(sumo_id not in vehicles_in_simu):
                if(sumo_id in traci.simulation.getArrivedIDList()):
                    dict_scenario[vehicle_type][i]["finish_step"] = step              
                else:
                ...
                if(traci.vehicle.getSpeed(sumo_id)< speed_threshold):
                    dict_scenario[vehicle_type][i]["waiting_time"] += 1

    #(step%1, step%1<=step_length)
    if(structure.open):
        structure.step(step, edges)

    print(f"\rStep {int(step)}: {len(traci.vehicle.getIDList())} vehicles in simu.",end="")
...
print("\nnumber of cars:", len(dict_scenario["cars"]), ",number of cyclists:", len(dict_scenario["bikes"]), "last step:", step)


mean_cars_travel_time = compute_graphs_data(dict_scenario, "cars")
mean_cyclists_travel_time = compute_graphs_data(dict_scenario, "bikes")
